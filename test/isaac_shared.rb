...
describe Crypt::ISAAC::DEFAULT do
  it 'Crypt::ISAAC::DEFAULT.srand and Crypt::ISAAC::DEFAULT.rand' do
    ...
    random_array_seed = []
    256.times { random_array_seed << ::Random.rand(4_294_967_296) }
...
describe Crypt::ISAAC do
  ...
  it 'the default generator exists, and it works' do
    mynum = Crypt::ISAAC.rand(1_000_000)
    mynum.must_be_kind_of Integer
    mynum.must_be :>, 0
    mynum.must_be :<, 1_000_000
  end
  ...
  it 'generates random numbers in the proscribed range when called ' \
    'with a range argument' do
  ...
  it 'performs as expected when generating a large quantity of numbers' do
    ...
    100_000.times do
      count += 1
      x = @generator.rand(4_294_967_295)
      print [x].pack('V').unpack('H8') if (count % 1_000).zero?
      if (count % 7_000).zero?
        print "\n"
      elsif (count % 1_000).zero?
        print ' '
      end
    end
    puts "\n"
  ...
  it 'multiple discrete number streams can be separately generated' do
    ...
    series_b = []
    1_000.times do
      series_a << first_generator.rand(4_294_967_295)
      series_b << second_generator.rand(4_294_967_295)
    end
  ...
  it 'benchmark' do
    ...
    start = Time.now
    Benchmark.bm do |bm|
      bm.report { 1_000_000.times { @generator.rand(4_294_967_295) } }
    end
    finish = Time.now
    puts "1000000 numbers generated in #{(finish - start)} seconds; " \
      "#{1_000_000 / (finish - start)} per second"

    puts "\nBenchmark float prng generation"
    start = Time.now
    Benchmark.bm { |bm| bm.report { 1_000_000.times { @generator.rand } } }
    finish = Time.now
    puts "1000000 numbers generated in #{(finish - start)} seconds; " \
      "#{1_000_000 / (finish - start)} per second\n"
    ...
    start = Time.now
    Benchmark.bm do |bm|
      bm.report { 1_000_000.times { @generator.rand(3..12) } }
    end
    finish = Time.now
    puts "1000000 numbers generated in #{(finish - start)} seconds; " \
      "#{1_000_000 / (finish - start)} per second\n"
